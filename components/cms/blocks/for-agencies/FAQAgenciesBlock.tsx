...
export function FAQAgenciesBlockComponent({ block, locale }: Props) {
  ...
  if (isArabic) {
    return (
      <section dir="rtl" className="py-20 lg:py-28 bg-secondary/30">
              ...
              item?.question && item?.answer ? (
                ...
                >
                  <button
                    ...
                    className="w-full py-6 flex items-start justify-between gap-4 text-right"
                  >
                    <span className="font-serif text-lg text-foreground text-right">
                      {item.question}
                    </span>
                  ...
                  >
                    <p className="text-muted-foreground leading-relaxed pr-0">{item.answer}</p>
                  </div>
                </div>
              ) : null,
            )}
